...
- `[HTML] DetectEncoding` ini key and *automatic* BOM / `<meta>` / file-content charset detection are **removed** — the web engine's built-in sniffing is the default. A **manual** per-view override ships with the `html-charset-override` change: right-click an HTML/MHT view → **Encoding** submenu → pick a code page. HTML is re-decoded **host-side** by transcoding the pristine cached bytes (Windows `MultiByteToWideChar`, Linux `QTextCodec`) then re-rendering; MHT keeps its page-side `TextDecoder` executor. Do **not** re-add the `OverrideEncoding` / `WebResourceRequested` interceptor in ad-hoc patches — automatic detection stays a separate dedicated change if ever justified.
