...
Files with **no BOM, no `<meta charset>` tag, and a non-UTF-8 encoding** (e.g. Windows-1251, KOI8-R) are decoded by the web engine's built-in sniffing, which usually assumes UTF-8 and may mis-render some characters. If you hit this, re-save the file as UTF-8 or add a `<meta charset="...">` tag. The old `[HTML] DetectEncoding` override was removed; bringing it back is tracked as [future-work item 1](openspec/notes/future-work.md).
