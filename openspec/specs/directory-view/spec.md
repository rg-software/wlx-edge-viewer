...
On Linux, dynamic shell thumbnails are not implemented (future-work #2 in `openspec/notes/future-work.md`); the `GenDirThumbs` key is silently ignored and the static `folder.png`/`file.png` icons are always used.
