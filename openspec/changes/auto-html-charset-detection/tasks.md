...
- [ ] 8.8 Manual verification on Linux (Double Commander, Qt Web Engine): MHT auto-correct + Auto-detect re-pick on a Qt build — HTML auto-detect verified on Linux (windows-1251 no-meta / wrong-meta / KOI8-R all re-decode to Cyrillic; UTF-8/ASCII render flicker-free). MHT auto-correct on Linux remains BLOCKED by a pre-existing Qt-WebEngine renderer freeze: a host-initiated `__evEncodingApply` re-render (second `mhtml2html.convert` on the page) freezes the renderer. Repro with the ORIGINAL (pre-singlebyte) loader: `encoding-wrong-charset.mht`/`large-page.mht` render a mojibake preview then the page hangs at ~1.5s; normal MHTs (`fileformatinfo.mht`, `decoo.mhtml`) render fine; the isolated page (no host roundtrip) shows detection posting `CMD_AUTO_ENCODING|windows-1251` correctly
