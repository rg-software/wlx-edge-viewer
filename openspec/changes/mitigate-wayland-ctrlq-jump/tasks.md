...
- [x] 2.1 In `EdgeViewer/EdgeLister_Linux.cpp` `EdgeLister::Create`, after the debug-log block and before `new QWidget(parent)`, compute `const bool isQuickView = (parent->parentWidget() != nullptr) && ((parent->windowFlags() & Qt::Window) != 0);`.
- [x] 2.2 When `isQuickView` is true, call `parent->setWindowFlags(parent->windowFlags() & ~Qt::Window);` followed by `parent->show();` (because `setWindowFlags` hides the widget). The existing container-creation logic (`new QWidget(parent)` + `QVBoxLayout` + `addWidget(view)` + `setFocusProxy` + `container->show()`) runs unchanged after this point.
- [x] 2.3 When `isQuickView` is false (F3 case), no change is made to the parent's flags — the existing code path runs untouched. Confirmed by code review of the diff: the F3 branch is byte-identical to before (only the debug-log block and the heuristic computation sit ahead of the unchanged `new LinuxBackend()` instantiation).
- [x] 2.4 The file's top-of-file comment block now describes the new `Qt::Window`-stripping step and why it is gated on `isQuickView`. The previous "avoids creating the QWebEngineView's compositor surface directly under DC's widget" claim was removed because the root-cause analysis showed that fix attempt did not actually avoid the jump — the escape is on DC's form, not on our widget.

*§1.2 (Wayland verification of the heuristic log output), §3 (Wayland verification of the fix on real DC), §4 (fallback if 3.7 regresses), §5 (Windows build unaffected), and §6 (documentation) remain; see the user's running of DC on Wayland for 1.2 + 3.x and the user/agent drive for 4.x/5/6.*
