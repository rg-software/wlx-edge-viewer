...
- `virtual-host-mapping`, `temp-file-management`, `eml`, `markdown`, `asciidoc`, `rst`, `mhtml`, `url-files`, `images`, `other-fallback`: no spec-level behavior change on Windows; on Linux they inherit the cross-platform rendering requirement from the new `linux-runtime` spec (the `Resources/assets/*` JS/CSS bundles load and render identically via either `WebView2Backend` or `WebKitBackend`).
