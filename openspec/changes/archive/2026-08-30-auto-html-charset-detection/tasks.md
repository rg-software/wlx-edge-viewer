...
- [x] 8.8 Manual verification on Linux (Double Commander, Qt Web Engine): MHT auto-correct + Auto-detect re-pick on a Qt build — verified via a dlopen driver against the installed plugin: HTML auto-detect (windows-1251 no-meta / wrong-meta / KOI8-R re-decode to Cyrillic; UTF-8/ASCII flicker-free), EML auto-detect, and MHT auto-correct all work on Linux. MHT needed two fixes: (1) single-byte `new TextDecoder` labels freeze the renderer → `mhtml/loader.html` now loads `singlebyte.js` and routes them through the JS table; (2) mhtml2html's decode-success path yields an empty `<body>` on Qt for legacy charsets → the forced re-render re-encodes the part to genuine UTF-8 (8-bit body, charset=utf-8) so mhtml2html takes its native UTF-8 path, which renders correctly on both Qt and WebView2. Verified: `encoding-wrong-charset.mht`, `large-page.mht` (auto-correct to Cyrillic, no freeze), `fileformatinfo.mht`, `decoo.mhtml`, and the manual 1252→Auto re-pick restores the detected render.
