...
Reuse the Markdown loader's navigation pattern (`_mdPushNav` equivalent, renamed for RST) so a click on a same-folder `.rst` link re-renders the target in-viewer via `compile()`; cross-folder links fall through. Navigation is **always-on** (anchors scroll to their target; same-folder `.rst` links render in-viewer) because RST is a cross-link-heavy format and the spec's RST Section Navigation requirement MUST be satisfied without an opt-in — only the floating back/forward history bar is gated on `NavigationUI=1`. (Correction from an earlier draft that gated the whole click handler on the `NavigationUI` flag: that made `__RST_NAVIGATION__` false by default since no `[RST] NavigationUI` key ships, so in-file anchors did not scroll and same-folder `.rst` clicks fell through to the browser, showing the raw source.)
