...
- [x] 7.4 Linux Double Commander: load the same `.rst` sample over the `ev://` scheme to confirm the bundle serves over the virtual host. **Verified via a dlopen driver against the installed plugin** (headless): `Examples/ReStructuredText.rst` renders over `ev://local.example/...`, in-file anchors work, and same-folder cross-file navigation (`RstNavTarget.rst`) swaps the content in-viewer. Cross-file navigation on Linux required replacing the loader's `fetch()` with an `XMLHttpRequest`-based load: Qt Web Engine's JS `fetch()` cannot reach custom schemes (Chromium's fetch allowlist ignores `registerScheme` — confirmed with and without the `FetchApiAllowed` flag; the handler is never invoked), while XHR works for `ev://`. The same fix was applied to the Markdown loader (`markdownExtension.js `_mdNavigate`) — Page A→B in-viewer confirmed — and both loaders keep their Windows `http://local.example` behavior (WebView2 intercepts XHR identically).
