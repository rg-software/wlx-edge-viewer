...
The lister SHALL route files whose extension matches the `EML` extension set from the `[Extensions]` section of `edgeviewer.ini` (e.g. `EML`) to the EML processor instead of any fallback processor. Detection SHALL be case-insensitive and SHALL behave identically on 32-bit and 64-bit builds.
