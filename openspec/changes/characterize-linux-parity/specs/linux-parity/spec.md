...
| 14 | Image rendering (SVG/PNG/JPG/ICO/WEBP) via `imgview/loader.html` | `works-verified` for render, **`broken-known` for the `F`-key FitToScreen toggle** | `manual-dc` | `Examples/Genaille_division_rods.svg` renders correctly. Pressing `F` does **not** toggle FitToScreen: the loader's keydown handler posts `window.chrome.webview.postMessage("CMD_ZOOM\|...")`, which is a WebView2-specific JS bridge and does not exist in Qt Web Engine. The CSS class swap happens (visible), but the host-side `put_ZoomFactor` is never called, so the resize observer does not change behavior. Spawn `fix-linux-image-fullscreen-toggle`. |
| 15 | PDF / Other rendering via `Navigate("http://local.example/...")` + Chromium PDF viewer | `broken-known` | `manual-dc` | `Examples/test-data/sample.pdf` renders as text/binary dump. Root cause: `EvSchemeHandler` (`QtWebEngineBackend.cpp:104-111`) only maps a small set of extensions; `.pdf` falls through to `text/html`, which prevents Chromium's built-in PDF viewer from activating. Fix: add `.pdf` → `application/pdf` (and probably `.zip`, `.docx`, `.odt`, etc. for completeness) to the MIME map. Spawn `fix-linux-scheme-mime-map`. |
| 16 | URL file → local file delegation (`UrlProcessor` → `LoadAndOpen` → `HtmlProcessor`) | `works-verified` for delegation | `manual-dc` | `Examples/test-data/sample-local.url` (target: `file:///.../sample.xhtml`) successfully chains through `UrlProcessor::OpenIn` → `gsProcRegistry().LoadAndOpen()` → `HtmlProcessor::OpenIn` and renders the file. The rendered result happens to be HTML (see Row 2 note on `EvSchemeHandler` default `text/html`). |
...
| 21 | Dark mode CSS injection (`[WebView]/[Type] CSSDark`) | `works-verified` *(for processors that go through `BaseFileProcessor` / pre-fetch path)*; **HTML processor separately broken-known** (see Row 24) | `manual-visual` + `auto-shared` (instrumented) | Manual test confirmed: switching KDE to dark theme correctly swaps `github.dark.css` for Markdown (`BaseFileProcessor::OpenIn` reads `[Markdown] CSSDark` at load time). The HTML processor uses a different path (`HtmlProcessor::OpenIn` → `Navigate`, no pre-fetch) and had its CSS injection wired only on Windows via `WebViewFactory::AddApplyStyleScript` — the Linux backend had no equivalent, see Row 24. **Root cause for the `gs_IsDarkMode`-always-false part** (now fixed by `fix-linux-dark-mode-fallback`): DC's Qt6 WLX caller does not propagate `lcp_darkmode` in `ShowFlags`. The fix added a `QGuiApplication::styleHints()->colorScheme()` fallback in `DllMain.cpp::ComputeDarkMode`. |
| 22 | Print (`ListPrintW` → `window.print()`) | `broken-known` | `manual-dc` | Manual test: Ctrl+P and the File menu's Print entry both have no effect. `Navigator::Print` calls `IWebView::ExecuteScript(L"window.print()")`, which on Linux is `page->runJavaScript(...)`. Modern Chromium suppresses `window.print()` (opens print preview) when the call is not from a user-gesture context; `runJavaScript` does not establish a gesture, so the preview is silently dropped. The Windows side has the same code path (`WebView2Backend::ExecuteScript` → `webView->ExecuteScript(...)`), but WebView2's print-to-PDF integration behaves differently. Fix paths: (a) call `QWebEnginePage::printToPdf(...)` directly to produce a PDF without the print dialog; (b) install a `PrintRequested` signal handler on the page and use `QPrintPreviewDialog` to surface the system print UI; (c) keep `window.print()` but trigger it via a synthetic user gesture (not directly supported by `runJavaScript`). Spawn `fix-linux-print`. |
...
- Linux ref: `EdgeViewer/WebView/QtWebEngineBackend.cpp:209-228` (Navigate passes URLs through) + `EvSchemeHandler` (`QtWebEngineBackend.cpp:104-111`) — the handler does not map `.pdf` and falls through to `Content-Type: text/html`. Chromium therefore displays the PDF bytes as text rather than invoking the PDF viewer.
- Status: `broken-known`.
- Test: `manual-dc` (open `Examples/test-data/sample.pdf`; expect Chromium's PDF viewer page chrome and a rendered first page; on the current build you get a text dump).
...
- Linux ref: `Resources/assets/imgview/loader.html:39-79` — same JS, but Qt Web Engine does not implement `window.chrome.webview`. The `postMessage` call resolves to `undefined` (or throws); the host never sees the toggle. The CSS class swap inside the page still happens, but the engine-level zoom is not synchronized.
- Status: `broken-known`.
- Test: `manual-dc` (open `Examples/Genaille_division_rods.svg`, press `F`; expect the image to flip between `full-screen` (fill viewport) and `real-size` (intrinsic pixels) classes — *visual swap works* — but the page zoom does not change. With the WebView2 zoom out of sync, the resize observer (`window.addEventListener('resize', ...)`) will fight the class on subsequent resizes.)
...
contain, fills viewport) and `real-size` (intrinsic dimensions). Visual
swap works on Linux; engine zoom does not synchronize.
...
- `gs_IsDarkMode` is set from `lcp_darkmode` (same on both platforms), but `prefers-color-scheme` propagation requires `QWebEngineProfile::setHttpCacheType` + palette hints. The `WebView2Backend` calls `put_PreferredColorScheme`; the `QtWebEngineBackend` does not. Subjective visual test on Linux dark mode is unverified — see Row 21.
