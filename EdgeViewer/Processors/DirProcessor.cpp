...
{ 
	...
	std::string loader(readFile(assetsPath() / L"dirviewer" / L"loader.html"));
	loader = std::regex_replace(loader, std::regex("__BASE_URL__"), urlPath(mPath.relative_path()));
	
	std::wstring wloader = to_utf16(loader);
...
std::wstring DirProcessor::genBody(const fs::path& path) const
{
	...
	for (auto const& dir_entry : fs::directory_iterator(path))
		if (dir_entry.is_regular_file())
		{
			...
			if (std::regex_match(fname, std::wregex(mask, std::regex_constants::icase)))
