...
void
sp_print_help(const char *basename)
{
	/* usage */
...
int
sp_parse_short_option(char opt, char *next, void *opaque)
{
...
int
sp_parse_long_option(char *opt, char *next, void *opaque)
{
	struct option_data *data = opaque;
	long int num;
	int isNum = next ? parseint(next, &num) : 0;
...
int
sp_parse_argument(int argn, char *arg, int is_forced, void *opaque)
{
	struct option_data *data = opaque;
...
smartypants_main(int argc, char **argv)
{
	...
	hoedown_buffer_puts(ib, SP_INPUT_STRING);
